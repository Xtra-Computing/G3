...
/*[[[cog
import json, cog
layers = json.load(open(jsonfile))['layers']
count_weight_matrices = 0
dim = {}
is_decay = {}
for layer in layers:
  if layer['name'].endswith('mul'):
    cog.outl('Array w' + str(count_weight_matrices) + ';')
    cog.outl('Array w' + str(count_weight_matrices) + '_grad;')
    dim[count_weight_matrices] = (layer['height'], layer['width']) # populating dimension information for weight matrices
    is_decay[count_weight_matrices] = layer['decay'] # populating whether weight matrix is decay
    count_weight_matrices += 1

# intermediate variable declarations
non_transformative_layers = {'input', 'output', 'relu', 'dropout', 'cross_entropy'}

get_output_varname = {}
get_output_varname['sprmul'] = lambda x: x + 'w0'
get_output_varname['graph_sum'] = lambda x: 'A' + x
get_output_varname['mat_mul'] = lambda x: x + 'w' + str(int(x[-1]) + 1) # only works for weight matrices with single digit index, e.g. w0 - w9
get_output_varname['relu'] = lambda x: x
get_output_varname['dropout'] = lambda x: x
get_output_varname['input'] = lambda x: x

varname = 'x'
for layer in layers:
  if layer['name'] in non_transformative_layers: continue
  varname = get_output_varname[layer['name']](varname)
  cog.outl('Array ' + varname + ';')
  cog.outl('Array ' + varname + '_grad;')
]]]*/
Array w0;
Array w0_grad;
Array w1;
...
//[[[end]]]
    ...
    Array in_feature, x_val;
    curandGenerator_t gen;
    util::GpuTimer timer;
    float tot_time = 0;
    float fw_dropout = 0, fw_sprmul = 0, fw_matmul = 0, fw_graphsum = 0, fw_relu = 0, fw_loss = 0;
...
//      loss.SetName ("loss");
      ...
      // weight matrices that needs to be initialized, can't stack more than 9 weight matrices
/*[[[cog
for i in range(count_weight_matrices):
  cog.outl('w' + str(i) + '.SetName("' + 'w' + str(i) + '");')
  cog.outl('w' + str(i) + '_grad.SetName("' + 'w' + str(i) + '_grad");')

# intermediate variable setname
varname = 'x'
for layer in layers:
  if layer['name'] in non_transformative_layers: continue
  varname = get_output_varname[layer['name']](varname)
  cog.outl(varname + '.SetName("' + varname + '");')
  cog.outl(varname + '_grad.SetName("' + varname + '_grad");')
]]]*/
w0.SetName("w0");
...
for i in range(count_weight_matrices):
  cog.outl('GUARD_CU(w' + str(i) + '.Allocate(' + str(dim[i][0]) + ' * ' + str(dim[i][1]) + '))')
  cog.outl('GUARD_CU(w' + str(i) + '_grad.Allocate(' + str(dim[i][0]) + ' * ' + str(dim[i][1]) + '))')

# allocate memory for intermediate variables
update_width = {}
update_width['sprmul'] = lambda layer, x: layer['width']
update_width['mat_mul'] = lambda layer, x: layer['width']
update_width['graph_sum'] = lambda layer, x: x

varname = 'x'
width = 'in_dim' # width of the current matrix
for layer in layers:
  if layer['name'] in non_transformative_layers: continue
  varname = get_output_varname[layer['name']](varname)
  width = update_width[layer['name']](layer, width)
...
//      GUARD_CU (truth.Print ())
      ...
      // random initializing weight matrices
      /*[[[cog
      for i in range(count_weight_matrices):
        cog.outl('curandGenerateUniformDouble(gen, w' + str(i) + '.GetPointer(util::DEVICE), w' + str(i) + '.GetSize ());')
        cog.outl('range = sqrt (6.0 / (' + str(dim[i][0]) + ' + ' + str(dim[i][1]) + '));')
        cog.outl('GUARD_CU (w' + str(i) + '.ForEach (')
        cog.outl('[range]__host__ __device__(ValueT &x) {')
        cog.outl('x = (x - 0.5) * range * 2;')
        cog.outl('}')
        cog.outl('))')
      ]]]*/
      ...
      def get_var_name_till(layers, i):
        varname = 'x'
        for j in range(0, i): varname = get_output_varname[layers[j]['name']](varname)
        return varname

      def f(layers, i):
        data = ''
        if layers[i - 1]['name'] == 'input':
          data = 'x.edge_values, dummy, '
        else:
          varname = get_var_name_till(layers, i)
          data = varname + ', &' + varname + '_grad, '
        return '<SizeT, ValueT>(' + data + str(layers[i]['rate']) + ', &gen, &fw_dropout, &bw_dropout)'
      get_call_arguments['dropout'] = f

      get_call_arguments['sprmul'] = lambda layers, i: '<SizeT, ValueT, SpmatT>(parameters, x, w0, w0_grad, xw0, xw0_grad, ' + str(layers[i]['height']) + ', ' + str(layers[i]['width']) + ', &fw_sprmul, &bw_sprmul)'

      def f(layers, i): 
        varname = get_var_name_till(layers, i)
        newvarname = get_output_varname[layers[i]['name']](varname)
        return '<SizeT, ValueT, GraphT>(parameters, sub_graph, ' + varname + ', ' + varname + '_grad, ' + newvarname + ', ' + newvarname + '_grad, ' + str(dim[int(varname[-1])][1]) + ', &fw_graphsum, &bw_graphsum)'
      get_call_arguments['graph_sum'] = f

      def f(layers, i): 
        varname = get_var_name_till(layers, i)
        return '<SizeT, ValueT>(' + varname + ', ' + varname + '_grad, num_nodes * ' + str(dim[int(varname[-1])][1]) + ', &fw_relu, &bw_relu)'
      get_call_arguments['relu'] = f

      def f(layers, i):
        varname = get_var_name_till(layers, i)
        newvarname = get_output_varname[layers[i]['name']](varname)
        return '<SizeT, ValueT>(' + varname + ', ' + varname + '_grad, ' + newvarname[-2:] + ', ' + newvarname[-2:] + '_grad, ' + newvarname + ', ' + newvarname + '_grad, num_nodes, ' + str(dim[int(varname[-1])][1]) + ', ' + str(dim[int(newvarname[-1])][-1]) + ', &fw_matmul, &bw_matmul)'
      get_call_arguments['mat_mul'] = f

      def f(layers, i):
        varname = get_var_name_till(layers, i)
        return '<SizeT, ValueT, GraphT>(parameters, ' + varname + ', ' + varname + '_grad, truth, num_nodes, ' + str(dim[int(varname[-1])][1]) + ', &fw_loss)'
      get_call_arguments['cross_entropy'] = f

      for i in range(1, len(layers)):
        layer = layers[i]
        cog.outl('modules.push_back(new ' + layer['name'] + get_call_arguments[layer['name']](layers, i) + ');')
        if layer['name'] == 'sprmul' and layers[i - 1]['name'] == 'dropout':
          cog.outl('x_val = static_cast<sprmul<SizeT, ValueT, SpmatT>*>(modules[' + str(i - 1) + '])->problem->')
          cog.outl('data_slices[0][0].sub_graph[0].SpmatT::CsrT::edge_values;')
      ...
      // reset array reference for dropout as sprmul would copy data and invalidate original array reference for previous dropout layer
      //x_val = static_cast<sprmul<SizeT, ValueT, SpmatT>*>(modules[1])->problem->
          //data_slices[0][0].sub_graph[0].SpmatT::CsrT::edge_values;
      //static_cast<dropout<SizeT, ValueT>*>(modules[0])->data = x_val;

      // copy input features as csr edge_value array
      ...
      /*[[[cog
      for i in range(count_weight_matrices):
        cog.outl('vars.emplace_back(w' + str(i) + ', w' + str(i) + '_grad, ' + is_decay[i] + ');')
      ]]]*/
  ...
  // TODO: change this into a code generation template
